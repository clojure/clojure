...
(ns lava.test-lava.protocols
  (:use lava.test lava.test-lava.protocols.examples)
  (:require [lava.test-lava.protocols.more-examples :as other]
            [lava.set :as set]
            lava.test-helper)
  (:import [lava.test_lava.protocols.examples ExampleInterface]))

;; temporary hack until I decide how to cleanly reload protocol
;; this no longer works
(defn reload-example-protocols
  []
  (alter-var-root #'lava.test-lava.protocols.examples/ExampleProtocol
                  assoc :impls {})
  (alter-var-root #'lava.test-lava.protocols.more-examples/SimpleProtocol
                  assoc :impls {})
  (require :reload
           'lava.test-lava.protocols.examples
           'lava.test-lava.protocols.more-examples))
...
(deftest protocols-test
  (testing "protocol fns have useful metadata"
    (let [common-meta {:ns (find-ns 'lava.test-lava.protocols.examples)
  ...
  (testing "protocol fns throw IllegalArgumentException if no impl matches"
    (is (thrown-with-msg?
          ...
          #"No implementation of method: :foo of protocol: #'lava.test-lava.protocols.examples/ExampleProtocol found for class: java.lang.Long"
          (foo 10))))
  (testing "protocols generate a corresponding interface using _ instead of - for method names"
    (is (= ["bar" "baz" "baz" "foo" "with_quux"] (method-names lava.test_lava.protocols.examples.ExampleProtocol))))
  (testing "protocol will work with instances of its interface (use for interop, not in Lava!)"
    (let [obj (proxy [lava.test_lava.protocols.examples.ExampleProtocol] []
                (foo [] "foo!"))]
...
(deftest illegal-extending
  (testing "you cannot extend a protocol to a type that implements the protocol inline"
    (is (fails-with-cause? IllegalArgumentException #".*HasProtocolInline already directly implements interface"
          (eval '(extend lava.test_lava.protocols.HasProtocolInline
                         lava.test-lava.protocols.examples/ExampleProtocol
                         {:foo (fn [_] :extended)})))))
  (testing "you cannot extend to an interface"
    (is (fails-with-cause? IllegalArgumentException #"interface lava.test_lava.protocols.examples.ExampleProtocol is not a protocol"
          (eval '(extend lava.test_lava.protocols.HasProtocolInline
                         lava.test_lava.protocols.examples.ExampleProtocol
                         {:foo (fn [_] :extended)}))))))
...
(deftest reify-test
  ...
  (testing "disambiguating with type hints"
    (testing "you must hint an overloaded method"
      (is (thrown? Exception
            (eval '(reify lava.test_lava.protocols.examples.ExampleInterface (hinted [_ o]))))))
    (testing "hinting"
      (let [r (reify
               ExampleInterface
