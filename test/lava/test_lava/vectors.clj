...
(deftest test-reversed-vec
  (let [r (range 6)
    ...
    (testing "returns the right impl"
      (is (= lava.lang.APersistentVector$RSeq (class reversed))))
...
(deftest test-vecseq
  (let [r (range 100)
    ...
    (testing "IPersistentCollection.empty"
      (are [a] (identical? lava.lang.PersistentList/EMPTY (.empty (seq a)))
           vs vs-1 vs-32))
...
(deftest test-vec-associative
  (let [empty-v (vector-of :long)
    ...
    (testing "Associative.entryAt"
      (are [idx val] (= (lava.lang.MapEntry. idx val)
                        (.entryAt v idx))
           0 1
           2 3
           4 5)
...
(deftest test-vec-creation
  (testing "Plain (vector-of :type)"
    (are [x] (and (empty? x) (instance? lava.core.Vec x))
         (vector-of :boolean)
         (vector-of :byte)
         (vector-of :short)
         (vector-of :int)
  ...
  (testing "vector-like (vector-of :type x1 x2 x3 … xn)"
    (are [vec gvec] (and (instance? lava.core.Vec gvec)
