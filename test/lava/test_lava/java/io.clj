...
(ns lava.test-lava.java.io
  (:use lava.test lava.java.io
...
(deftest test-spit-and-slurp
  (let [f (temp-file "lava.java.io" "test")]
    (spit f "foobar")
    (is (= "foobar" (slurp f)))
...
(deftest test-streams-defaults
  (let [f (temp-file "lava.java.io" "test-reader-writer")
        content "testing"]
    (try
...
(defn bytes-should-equal [byte-array-1 byte-array-2 msg]
  (is (= @#'lava.java.io/byte-array-type (class byte-array-1) (class byte-array-2)) msg)
...
(deftest test-resource
  ...
  (is (instance? URL (resource "lava/core.clj")))
