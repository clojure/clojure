...
This implementation for lava has the following goals:

 * Support the full feature set of the Common Lisp format function (including the X3J13 extensions) with the only exception being concepts that make no sense or are differently interpreted in Lava.
 * Make porting code from Common Lisp easier.
 * Provide a more native feeling solution for Lava programmers than the Java format method and its relatives.
...
The corresponding output using Lava's Java-based _format_ function
...
Once cl-format is in your path, adding it to your code is easy:
    ...
    (ns your-namespace-here
      (:use [lava.pprint :only (cl-format)]))

If you want to refer to the cl-format function as "format" (rather
than using the lava function of that name), you can use this idiom:
...
cl-format is a standard lava function that takes a variable number
...
that's similar to (but much more powerful than) the standard Lava
...
with the native lava calls like print. If you want stay
...
_exponentchar_).  Lava does not distinguish between floats and
...
() and nil are not equivalent in Lava.

Lava has 3 different reader syntaxes for characters. The ~@c
