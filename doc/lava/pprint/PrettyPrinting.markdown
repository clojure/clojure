# A Pretty Printer for Lava

## Overview

This namespace adds a new feature to Lava: a generalized pretty
printer.
...
default) which formats the various Lava data structures as
...
lava.pprint namespace. Using them is as simple as adding a 
`(:use lava.pprint)` to
your namespace declarations. Or, better practice would be 
`(:use [lava.pprint :only (<functions you wish to use>)])`.

pprint is being developed by Tom Faulhaber (to mail me you can use
my first name at my domain which is infolace.com).

As with the rest of Lava, the pretty printer is licensed under the 
...
Lava structures: seqs, maps, vectors, etc. in a fairly statndard
...
As with the regular Lava print function, this variable controls the 
...
how to use custom dispatch to translate simple Lava structures into
...
the fundamentals of pretty printing. The lava pretty printer is
...
The Lava implementation of XP is similar in spirit to the Common
Lisp implementation, but the details of the interface are somewhat
different. The result is that writing custom dispatch in Lava is
more "Lava-y."
 ...
 * Support all the types and forms in Lava (most of the way there now).
