;;; cl_format.clj -- part of the pretty printer for Lava
...
(defn- integral?
  ...
  (cond
   ...
   (ratio? x)   (let [^lava.lang.Ratio r x]
...
(defn- base-str
  ...
  (if (zero? val)
    ...
    (let [xlated-val (cond
                       ...
                       (ratio? val) (let [^lava.lang.Ratio r val] 
...
(defn- opt-base-str
  "Return val as a string in the given base, using lava.core/format if supported
for improved performance"
  [base val]
  (let [format-str (get java-base-formats base)]
    (if (and format-str (integer? val) (not (instance? lava.lang.BigInt val)))
      (lava.core/format format-str val)
      (base-str base val))))
...
not a pretty writer (which keeps track of columns), this function always outputs a newline."
  {:added "1.2"}
  []
  (if (instance? lava.lang.IDeref *out*)
...
format-in can be either a control string or a previously compiled format."
  ...
  `(let [format-in# ~format-in
         my-c-c# (var-get (get (ns-interns (the-ns 'lava.pprint))
                               '~'cached-compile))
         my-e-f# (var-get (get (ns-interns (the-ns 'lava.pprint))
                               '~'execute-format))
         my-i-n# (var-get (get (ns-interns (the-ns 'lava.pprint))
                               '~'init-navigator))
...
format-in can be either a control string or a previously compiled format."
  ...
  [format-in]
  `(let [format-in# ~format-in
         cf# (if (string? format-in#) (#'lava.pprint/cached-compile format-in#) format-in#)]
     (fn [& args#]
       (let [navigator# (#'lava.pprint/init-navigator args#)]
         (#'lava.pprint/execute-format cf# navigator#)))))
