;;; pprint_base.clj -- part of the pretty printer for Lava
...
;;;
;;; *print-length*, *print-level* and *print-dup* are defined in lava.core
...
(def ^{:private true} write-option-table
     {;:array            *print-array*
      :base             'lava.pprint/*print-base*,
      ;;:case             *print-case*,
      :circle           'lava.pprint/*print-circle*,
      ;;:escape           *print-escape*,
      ;;:gensym           *print-gensym*,
      :length           'lava.core/*print-length*,
      :level            'lava.core/*print-level*,
      :lines            'lava.pprint/*print-lines*,
      :miser-width      'lava.pprint/*print-miser-width*,
      :dispatch         'lava.pprint/*print-pprint-dispatch*,
      :pretty           'lava.pprint/*print-pretty*,
      :radix            'lava.pprint/*print-radix*,
      :readably         'lava.core/*print-readably*,
      :right-margin     'lava.pprint/*print-right-margin*,
      :suppress-namespaces 'lava.pprint/*print-suppress-namespaces*})


(defmacro ^{:private true} binding-map [amap & body]
  (let []
    `(do
       (. lava.lang.Var (pushThreadBindings ~amap))
       (try
        ~@body
        (finally
...
(defn- pretty-writer? 
  ...
  [x] (and (instance? lava.lang.IDeref x) (:pretty-writer @@x)))
...
and :suffix."
  ...
  (let [[options body] (parse-lb-options #{:prefix :per-line-prefix :suffix} args)]
    `(do (if (#'lava.pprint/level-exceeded) 
           (.write ^java.io.Writer *out* "#")
           (do 
             (push-thread-bindings {#'lava.pprint/*current-level*
                                    (inc (var-get #'lava.pprint/*current-level*))
                                    #'lava.pprint/*current-length* 0})
             (try  
              (#'lava.pprint/start-block *out*
                           ~(:prefix options) ~(:per-line-prefix options) ~(:suffix options))
              ~@body
              (#'lava.pprint/end-block *out*)
              (finally 
               (pop-thread-bindings)))))
         nil)))
