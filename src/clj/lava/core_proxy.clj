...
(import
 '(lava.asm ClassWriter ClassVisitor Opcodes Type) 
 '(java.lang.reflect Modifier Constructor)
 '(lava.asm.commons Method GeneratorAdapter)
 '(lava.lang IProxy Reflector DynamicClassLoader IPersistentMap PersistentHashMap RT))
...
(defn- generate-proxy [^Class super interfaces]
  (let [cv (new ClassWriter (. ClassWriter COMPUTE_MAXS))
        cname (.replace (proxy-name super interfaces) \. \/) ;(str "lava/lang/" (gensym "Proxy__"))
        ...
        ifn-type (totype lava.lang.IFn)
        obj-type (totype Object)
        sym-type (totype lava.lang.Symbol)
        rt-type  (totype lava.lang.RT)
            ...
            (let [pclasses (. meth (getParameterTypes))
              ...
              (if (> (count pclasses) 18)
                ...
                (do
                  ...
                  (dotimes [i (count ptypes)]
                      (. gen (loadArg i))
                    (. lava.lang.Compiler$HostExpr (emitBoxReturn nil gen (nth pclasses i))))
                                        ;call fn
    ...
    (doseq [^Constructor ctor (. super (getDeclaredConstructors))]
        (when-not (. Modifier (isPrivate (. ctor (getModifiers))))
          (let [ptypes (to-types (. ctor (getParameterTypes)))
            ...
            (. gen (endMethod)))))
                                        ;add IProxy methods
    (let [m (. Method (getMethod "void __initLavaFnMappings(lava.lang.IPersistentMap)"))
          gen (new GeneratorAdapter (. Opcodes ACC_PUBLIC) m nil nil cv)]
      (. gen (visitCode))
      (. gen (loadThis))
      (. gen (loadArgs))
      ...
      (. gen (returnValue))
      (. gen (endMethod)))
    (let [m (. Method (getMethod "void __updateLavaFnMappings(lava.lang.IPersistentMap)"))
          gen (new GeneratorAdapter (. Opcodes ACC_PUBLIC) m nil nil cv)]
      (. gen (visitCode))
      (. gen (loadThis))
      (. gen (dup))
      (. gen (getField ctype fmap imap-type))
      (.checkCast gen (totype lava.lang.IPersistentCollection))
      (. gen (loadArgs))
      (. gen (invokeInterface (totype lava.lang.IPersistentCollection)
                              (. Method (getMethod "lava.lang.IPersistentCollection cons(Object)"))))
      (. gen (checkCast imap-type))
      (. gen (putField ctype fmap imap-type))
      
      (. gen (returnValue))
      (. gen (endMethod)))
    (let [m (. Method (getMethod "lava.lang.IPersistentMap __getLavaFnMappings()"))
          gen (new GeneratorAdapter (. Opcodes ACC_PUBLIC) m nil nil cv)]
      (. gen (visitCode))
      (. gen (loadThis))
      (. gen (getField ctype fmap imap-type))
    ...
    (let [[super interfaces] (get-super-and-interfaces bases)
      ...
      (or (RT/loadClassForName pname)
          (let [[cname bytecode] (generate-proxy super interfaces)]
            (. ^DynamicClassLoader (deref lava.lang.Compiler/LOADER) (defineClass pname bytecode [super interfaces]))))))
...
(defn init-proxy
  ...
  [^IProxy proxy mappings]
    (. proxy (__initLavaFnMappings mappings))
    proxy)
...
(defn update-proxy
  ...
  {:added "1.0"}
  [^IProxy proxy mappings]
    (. proxy (__updateLavaFnMappings mappings))
    proxy)
  ...
  [class-and-interfaces args & fs]
   (let [bases (map #(or (resolve %) (throw (Exception. (str "Can't resolve: " %)))) 
         ...
         compile-effect (when *compile-files*
                          (let [[cname bytecode] (generate-proxy super interfaces)]
                            (lava.lang.Compiler/writeClassFile cname bytecode)))
	...
	pmap (reduce1 (fn [m ^java.beans.PropertyDescriptor pd]
			 (let [name (. pd (getName))
			   ...
			   (if (and method (zero? (alength (. method (getParameterTypes)))))
			     (assoc m (keyword name) (fn [] (lava.lang.Reflector/prepRet (.getPropertyType pd) (. method (invoke x nil)))))
			     m)))
		     {}
	...
	v (fn [k] ((pmap k)))
        snapshot (fn []
                   (reduce1 (fn [m e]
                             (assoc m (key e) ((val e))))
                           {} (seq pmap)))]
    (proxy [lava.lang.APersistentMap]
           []
      (containsKey [k] (contains? pmap k))
      (entryAt [k] (when (contains? pmap k) (new lava.lang.MapEntry k (v k))))
      (valAt ([k] (v k))
      ...
      (seq [] ((fn thisfn [plseq]
		  (lazy-seq
                   (when-let [pseq (seq plseq)]
                     (cons (new lava.lang.MapEntry (first pseq) (v (first pseq)))
