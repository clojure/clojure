...
(defn- generate-class [options-map]
  ...
  (let [default-options {:prefix "-" :load-impl-ns true :impl-ns (ns-name *ns*)}
        ...
        main-name "main"
        var-name (fn [s] (lava.lang.Compiler/munge (str s "__var")))
        class-type  (totype Class)
        rt-type  (totype lava.lang.RT)
        var-type ^Type (totype lava.lang.Var)
        ifn-type (totype lava.lang.IFn)
        iseq-type (totype lava.lang.ISeq)
    ...
    (let [gen (new GeneratorAdapter (+ (. Opcodes ACC_PUBLIC) (. Opcodes ACC_STATIC)) 
      ...
      (doseq [v var-fields]
        ...
        (. gen (invokeStatic var-type (. Method (getMethod "lava.lang.Var internPrivate(String,String)"))))
        (. gen putStatic ctype (var-name v) var-type))
      
      (when load-impl-ns
        (. gen push "lava.core")
        (. gen push "load")
        (. gen (invokeStatic rt-type (. Method (getMethod "lava.lang.Var var(String,String)"))))
...
;        (. gen (invokeStatic class-type (. Method (getMethod "Class forName(String)"))))
        ...
        (if init
          (do
            ...
            (dotimes [i (count super-pclasses)]
              ...
              (. gen push (int i))
              (. gen (invokeStatic rt-type nth-method))
              (. lava.lang.Compiler$HostExpr (emitUnboxArg nil gen (nth super-pclasses i))))
    ...
    (when main
      (let [m (. Method getMethod "void main (String[])")
        ...
        (.checkCast gen ifn-type)
        (. gen loadArgs)
        (. gen (invokeStatic rt-type (. Method (getMethod "lava.lang.ISeq seq(Object)"))))
...
(defmacro gen-class 
  ...
  functions in an implementing Lava namespace. Given a generated
  class org.mydomain.MyClass with a method named mymethod, gen-class
  will generate an implementation that looks for a function named by 
  (str prefix mymethod) (default prefix: \"-\") in a
  Lava namespace specified by :impl-ns
  (defaults to the current namespace). All inherited methods,
  generated methods, and init and main functions (see :methods, :init,
  and :main below) will be found similarly prefixed. By default, the
  static initializer for the generated class will attempt to load the
  Lava support code for the class as a resource from the classpath,
  ...
  occur in Lava functions, they have no access to the inherited
...
(defmacro gen-interface
  ...
  [& options]
    (let [options-map (apply hash-map options)
      ...
      (if *compile-files*
        (lava.lang.Compiler/writeClassFile cname bytecode)
        (.defineClass ^DynamicClassLoader (deref lava.lang.Compiler/LOADER) 
...
(defn gen-and-load-class 
  ...
  (let [options-map (apply hash-map options)
        [cname bytecode] (generate-class options-map)]
    (.. (lava.lang.RT/getRootClassLoader) (defineClass cname bytecode options))))
