...
(ns ^{:doc "Top-level main function for Lava REPL and scripts."
       :author "Stephen C. Gilardi and Rich Hickey"}
  lava.main
  (:refer-lava :exclude [with-bindings])
  (:import (lava.lang Compiler Compiler$CompilerException
                         LineNumberingPushbackReader RT))
  ;;(:use [lava.repl :only (demunge root-cause stack-element-str)])
  )

(declare main)

;;;;;;;;;;;;;;;;;;; redundantly copied from lava.repl to avoid dep ;;;;;;;;;;;;;;
...
(def ^:private demunge-map
  (into {"$" "/"} (map (fn [[k v]] [v k]) lava.lang.Compiler/CHAR_MAP)))
...
(defn root-cause
  ...
  [^Throwable t]
  (loop [cause t]
    (if (and (instance? lava.lang.Compiler$CompilerException cause)
             (not= (.source ^lava.lang.Compiler$CompilerException cause) "NO_SOURCE_FILE"))
      cause
...
(defn stack-element-str
  ...
  (let [file (.getFileName el)
        lava-fn? (and file (or (.endsWith file ".clj")
                                  (= file "NO_SOURCE_FILE")))]
    (str (if lava-fn?
           (demunge (.getClassName el))
           (str (.getClassName el) "." (.getMethodName el)))
         " (" (.getFileName el) ":" (.getLineNumber el) ")")))
;;;;;;;;;;;;;;;;;;; end of redundantly copied from lava.repl to avoid dep ;;;;;;;;;;;;;;
...
(defmacro with-bindings
  ...
  *command-line-args* *1 *2 *3 *e"
  ...
  `(binding [*ns* *ns*
             ...
             *compile-path* (System/getProperty "lava.compile.path" "classes")
  ...
  (let [ex (repl-exception e)
    ...
    (binding [*out* *err*]
      (println (str (-> ex class .getSimpleName)
                    " " (.getMessage ex) " "
                    (when-not (instance? lava.lang.Compiler$CompilerException ex)
...
(defn repl
  ...
  (let [cl (.getContextClassLoader (Thread/currentThread))]
    (.setContextClassLoader (Thread/currentThread) (lava.lang.DynamicClassLoader. cl)))
    ...
    (with-bindings
     (try
      ...
      (catch Throwable e
        (caught e)
        (set! *e e)))
     (use '[lava.repl :only (source apropos dir pst doc find-doc)])
     (use '[lava.java.javadoc :only (javadoc)])
     (use '[lava.pprint :only (pp pprint)])
     (prompt)
     (flush)
     (loop []
       (when-not 
...
(defn load-script
  "Loads Lava source from a file or resource given its path. Paths
...
(defn- legacy-repl
  "Called by the lava.lang.Repl.main stub to run a repl with args
  specified the old way"
  [args]
  (println "WARNING: lava.lang.Repl is deprecated.
Instead, use lava.main like this:
java -cp lava.jar lava.main -i init.clj -r args...")
  (let [[inits [sep & args]] (split-with (complement #{"--"}) args)]
    (repl-opt (concat ["-r"] args) (map vector (repeat "-i") inits))))

(defn- legacy-script
  "Called by the lava.lang.Script.main stub to run a script with args
  specified the old way"
  [args]
  (println "WARNING: lava.lang.Script is deprecated.
Instead, use lava.main like this:
java -cp lava.jar lava.main -i init.clj script.clj args...")
  (let [[inits [sep & args]] (split-with (complement #{"--"}) args)]
    (null-opt args (map vector (repeat "-i") inits))))

(defn main
  "Usage: java -cp lava.jar lava.main [init-opt*] [main-opt] [arg*]
  ...
  suppresses the usual repl greeting message: \"Lava ~(lava-version)\".
