...
;; Java-specific parts of lava.reflect
(in-ns 'lava.reflect)

(require '[lava.set :as set]
         '[lava.string :as str])
(import '[lava.asm ClassReader ClassVisitor Type]
...
(defn- typesym
  "Given a typeref, create a legal Lava symbol version of the
   type's name."
  [t]
  (-> (typename t)
      (str/replace "[]" "<>")
...
(defn- field-descriptor->class-symbol
  "Convert a Java field descriptor to a Lava class symbol. Field
...
(defn- internal-name->class-symbol
  "Convert a Java internal name to a Lava class symbol. Internal
...
(defn- declared-constructors
  "Return a set of the declared constructors of class as a Lava map."
  [^Class cls]
  (set (map
        constructor->map
...
(defn- declared-fields
  "Return a set of the declared fields of class as a Lava map."
  [^Class cls]
  (set (map
        field->map
