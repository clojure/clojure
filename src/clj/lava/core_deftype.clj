...
(defn namespace-munge
  "Convert a Lava namespace name to a legal Java package name."
  {:added "1.2"}
  [ns]
  (.replace (str ns) \- \_))
...
(defn hash-combine [x y] 
  (lava.lang.Util/hashCombine x (lava.lang.Util/hash y)))

(defn munge [s]
  ((if (symbol? s) symbol str) (lava.lang.Compiler/munge (str s))))
...
(defn- imap-cons
  ...
  (cond
   ...
   (instance? lava.lang.IPersistentVector o)
     (let [^lava.lang.IPersistentVector vec o]
  ...
  (let [tag (keyword (str *ns*) (str tagname))
    ...
    (letfn 
     [(eqhash [[i m]] 
        [i
         (conj m 
               `(hashCode [this#] (lava.lang.APersistentMap/mapHash this#))
               `(equals [this# ~gs] (lava.lang.APersistentMap/mapEquals this# ~gs)))])
      (iobj [[i m]] 
            [(conj i 'lava.lang.IObj)
             (conj m `(meta [this#] ~'__meta)
                   `(withMeta [this# ~gs] (new ~tagname ~@(replace {'__meta gs} fields))))])
      (ilookup [[i m]] 
         [(conj i 'lava.lang.ILookup 'lava.lang.IKeywordLookup)
                ...
                `(getLookupThunk [this# k#]
                   (let [~'gclass (class this#)]              
                     (case k#
                           ...
                           nil))))])
      (imap [[i m]] 
            [(conj i 'lava.lang.IPersistentMap)
             (conj m 
                   ...
                   `(entryAt [this# k#] (let [v# (.valAt this# k# this#)]
                                            (when-not (identical? this# v#)
                                              (lava.lang.MapEntry. k# v#))))
                   `(seq [this#] (seq (concat [~@(map #(list `new `lava.lang.MapEntry (keyword %) %) base-fields)] 
                                              ~'__extmap)))
                   `(assoc [this# k# ~gs]
                     (condp identical? k#
                       ~@(mapcat (fn [fld]
...
(defmacro defrecord
  ...
  The class will have implementations of several (lava.lang)
...
(defmacro deftype
  ...
  level constructs, such as Lava's reference types, in Lava
...
(defn- expand-method-impl-cache [^lava.lang.MethodImplCache cache c f]
  (let [cs (into1 {} (remove (fn [[c e]] (nil? e)) (map vec (partition 2 (.table cache)))))
        cs (assoc cs c (lava.lang.MethodImplCache$Entry. c f))
        ...
        table (reduce1 (fn [^objects t [c e]]
                        (let [i (* 2 (int (shift-mask shift mask (hash c))))]
                          ...
                          t))
                      table cs)]
    (lava.lang.MethodImplCache. (.protocol cache) (.methodk cache) shift mask table)))
...
(defn -cache-protocol-fn [^lava.lang.AFunction pf x ^Class c ^lava.lang.IFn interf]
...
(defn- emit-method-builder [on-interface method on-method arglists]
  (let [methodk (keyword method)
        gthis (with-meta (gensym) {:tag 'lava.lang.AFunction})
        ginterf (gensym)]
    `(fn [cache#]
       (let [~ginterf
             (fn
                    ...
                    (let [gargs (map #(gensym (str "gf__" % "__")) args)
                      ...
                      `([~@gargs]
                          (let [cache# (.__methodImplCache ~gthis)
                                f# (.fnFor cache# (lava.lang.Util/classOf ~target))]
                            (if f# 
                              (f# ~@gargs)
...
(defn -reset-methods [protocol]
  (doseq [[^lava.lang.Var v build] (:method-builders protocol)]
    (let [cache (lava.lang.MethodImplCache. protocol (keyword (.sym v)))]
...
(defmacro extend-protocol 
  ...
  (do
   (lava.core/extend-type AType Protocol 
     (foo [x] ...) 
     (bar [x y] ...))
   (lava.core/extend-type BType Protocol 
     (foo [x] ...) 
     (bar [x y] ...))
   (lava.core/extend-type AClass Protocol 
     (foo [x] ...) 
     (bar [x y] ...))
   (lava.core/extend-type nil Protocol 
     (foo [x] ...) 
     (bar [x y] ...)))"
  {:added "1.2"}
