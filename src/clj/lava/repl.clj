...
(defn- print-doc [m]
  ...
  (if (:special-form m)
    (do
      ...
      (if (contains? m :url)
        (when (:url m)
          (println (str "\n  Please see http://lava.org/" (:url m))))
        (println (str "\n  Please see http://lava.org/special_forms#"
                      (:name m)))))
    (do
      (when (:macro m)
        (println "Macro")) 
...
;; ----------------------------------------------------------------------
;; Examine Lava functions (Vars, really)
  ...
  ([^Throwable e depth]
     (binding [*out* *err*]
       ...
       (let [st (.getStackTrace e)
             cause (.getCause e)]
         (doseq [el (take depth
                          (remove #(#{"lava.lang.RestFn" "lava.lang.AFn"} (.getClassName %))
                                  st))]
